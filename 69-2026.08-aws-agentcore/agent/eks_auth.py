...
def get_api_client(cluster_name: str, region: str) -> k8s_client.ApiClient:
    """Build a fresh kubernetes ApiClient authenticated against the given EKS cluster.

    Called per-tool-invocation (not cached) so the bearer token never goes
    stale across a longer agent session.
    """
